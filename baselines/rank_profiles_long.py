...
from prompts.utils import extract_strings_between_quotes, extract_after_article, extract_after_review, \
    extract_after_colon, extract_after_description, \
    extract_after_post, extract_after_description2, extract_after_title
...
parser.add_argument("--contriever_checkpoint", default="../models/contriever")
...
def generation_topic_query_corpus_maker(inp, profile):
    corpus = [f'{x["content"]} {x["summary"]}' for x in profile]
    ids = [x['id'] for x in profile]
    query = extract_after_post(inp)
    return corpus, query, ids


def generation_review_query_corpus_maker(inp, profile):
    corpus = [f'{x["reviewText"]} {x["description"]}' for x in profile]
    # ids = [x['id'] for x in profile]
    query = extract_after_description2(inp)
    return corpus, query


def generation_abstract_query_corpus_maker(inp, profile):
    corpus = [f'{x["abstract"]} {x["title"]}' for x in profile]
    ids = [x['id'] for x in profile]
    query = extract_after_title(inp)
    return corpus, query, ids
...
if __name__ == "__main__":
    ...
    for data in tqdm.tqdm(dataset):
        ...
        elif task == "TopicWriting":
            corpus, query, ids = generation_topic_query_corpus_maker(inp, profile)
        elif task == "ProductReview":
            corpus, query = generation_review_query_corpus_maker(inp, profile)
        elif task == "AbstractGeneration":
            corpus, query, ids = generation_abstract_query_corpus_maker(inp, profile)
