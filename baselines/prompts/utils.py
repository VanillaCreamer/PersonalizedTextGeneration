...
def extract_after_post(input_string):
    article_index = input_string.find('reddit post')
    if article_index == -1:
        return None
    return input_string[article_index + len('reddit post'):].strip()


def extract_after_description2(input_string):
    article_index = input_string.find('description')
    if article_index == -1:
        return None
    return input_string[article_index + len('description'):].strip()
...
def extract_after_title(input_string):
    article_index = input_string.find('the title')
    if article_index == -1:
        return None
    return input_string[article_index + len('the title'):].strip()
