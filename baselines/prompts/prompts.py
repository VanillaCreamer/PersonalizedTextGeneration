...
from LaMP.prompts.utils import extract_strings_between_quotes, extract_after_article, extract_after_review, extract_after_paper, add_string_after_title, extract_after_colon, extract_after_description, extract_after_abstract, extract_after_post
from LaMP.prompts.contriever_retriever import retrieve_top_k_with_contriever
import random
...
def create_generation_topic_prompt(inp, profile, max_length, tokenizer):
    """
    :param inp: original input
    :param profile: top-k profiles
    ...
    prompts = []
    for p in profile:
        needed_part_len = len(tokenizer(f'"{p["summary"]}" is a summary " " ')['input_ids'])
        tokens = tokenizer(p["content"], max_length=per_p_max_length + saved_tokens - needed_part_len, truncation=True)
        saved_tokens += per_p_max_length - len(tokens['input_ids']) - needed_part_len
        new_asbtract = tokenizer.batch_decode([tokens['input_ids']], skip_special_tokens=True)[0]
        prompt = f'"{p["summary"]}" is a summary for "{new_asbtract}" '
        prompts.append(prompt)
    return f'{", and ".join(prompts)}. Following the given patterns {inp}'
...
def create_prompt_generator(num_retrieve, ret_type = "bm25", is_ranked = False, max_length = 512, tokenizer = None):
    ...
    def prompt(inp, profile, task):
        if task == "LaMP-1":
            ...
            corpus, query = generation_topic_query_corpus_maker(inp, profile)
