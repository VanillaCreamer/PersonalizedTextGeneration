...
def pos_aggregator() -> Aggregator:

    def _pos_aggregator(pos_acts: Tensor, neg_acts: Tensor) -> Tensor:
        return (pos_acts).mean(dim=0)

    return _pos_aggregator

def attention_aggregator(attentions) -> Aggregator:
    """
    An aggregator that uses the attention mechanism to calculate a steering vector. The steering vector of  documents with high attention values will have higher weights.
    """
    @torch.no_grad()
    def _attention_aggregator(pos_acts: Tensor, neg_acts: Tensor) -> Tensor:
        deltas = pos_acts - neg_acts
        weighted_deltas = deltas * attentions
        return weighted_deltas.mean(dim=0)
    return _attention_aggregator
...
def pca_aggregator() -> Aggregator:
    ...
    def _pca_aggregator(pos_acts: Tensor, neg_acts: Tensor) -> Tensor:
        print(pos_acts.shape, neg_acts.shape)
        deltas = pos_acts - neg_acts
        neg_deltas = -1 * deltas
...
def _uncentered_pca(data: Tensor, k: int = 1) -> Tensor:
    ...
    data_fp32 = data.to(dtype=torch.float32)
    u, _s, _v = torch.svd(torch.t(data_fp32))
    result = u[:, :k]
